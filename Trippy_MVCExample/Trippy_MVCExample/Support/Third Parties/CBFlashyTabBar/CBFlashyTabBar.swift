...
open class CBFlashyTabBar: UITabBar {
    ...
    public var animationSpeed: Double = 0.5 {
        didSet {
            reloadAnimations()
        }
    }
    ...
    private func reloadAnimations() {
        buttons.forEach { (button) in
            button.selectAnimation = CBTabItemSelectAnimation(duration: 0.1 / animationSpeed)
            button.deselectAnimation = CBTabItemDeselectAnimation(duration: 0.1 / animationSpeed)
        }
    }
